...
{
  "displayName": "Facebook Pixel (v2.0)",
  "__wm": "VGVtcGxhdGUtQXV0aG9yX0ZhY2Vib29rLVNpbW8tQWhhdmE\u003d",
  "description": "A flexible Facebook Pixel tag for Google Tag Manager that adds automatic Enhanced Ecommerce (GA4, UA, eventModel) tracking support — while still supporting your existing manual setups.",
  "categories": [
    "ADVERTISING",
    "ANALYTICS"
  ],
...
[
  ...
  {
    "type": "SELECT",
    "name": "eecDataLayerFormat",
    "displayName": "EEC Data Layer Format",
    ...
    "selectItems": [
      ...
      {
        "value": "ua",
        "displayValue": "UA (ecommerce.*)"
      },
      {
        "value": "eventModel",
        "displayValue": "eventModel"
      }
    ],
    "simpleValueType": true,
    "enablingConditions": [
  ...
  {
    "type": "CHECKBOX",
    "name": "enableEventRemap",
    "checkboxText": "Enable Custom to GA4 Event Mapping",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "enhancedEcommerce",
  ...
  },
  {
    ...
    "simpleTableColumns": [
      {
        "defaultValue": "",
        "displayName": "Your Event Name (site)",
        "name": "siteEventName",
        "type": "TEXT"
      },
      {
  ...
  {
    "type": "CHECKBOX",
    "name": "eecCustomCurrencyMap",
    "checkboxText": "Custom currency field key",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "enhancedEcommerce",
  ...
  {
    "type": "TEXT",
    "name": "eecCustomCurrency",
    "displayName": "",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "eecCustomCurrencyMap",
        "paramValue": true,
        "type": "EQUALS"
      }
    ]
  ...
  {
    "type": "CHECKBOX",
    "name": "eecCustomContentIdMap",
    "checkboxText": "Content ID field key",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "enhancedEcommerce",
  ...
  {
    "type": "TEXT",
    "name": "eecCustomContentId",
    "displayName": "",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "eecCustomContentIdMap",
        "paramValue": true,
        "type": "EQUALS"
      }
    ]
  ...
  },
  {
    "type": "TEXT",
    "name": "stripPrefix",
    "displayName": "Remove Prefix from Event Name (e.g. \u0027dl_\u0027 from \u0027dl_view_item\u0027)",
    "simpleValueType": true,
    "enablingConditions": [
      {
        "paramName": "stripPrefixBox",
        "paramValue": true,
        "type": "EQUALS"
      }
    ]
...
// EEC
var eventName, eecObjectProps;

if (data.enhancedEcommerce) {

	var gtmEvent = copyFromDataLayer('event');
	var ecommerce = null;

	const format = data.eecDataLayerFormat;

	log('📦 eecDataLayerFormat:', format);


	if (format == 'ga4') {
		ecommerce = copyFromDataLayer('ecommerce', 1);
	} else if (format == 'eventModel') {
		ecommerce = copyFromDataLayer('eventModel');
	} else if (format == 'ua') {
		ecommerce = copyFromDataLayer('ecommerce', 1);
	}

	log('📦 Final ecommerce:', ecommerce);

	if (!ecommerce) {
		return fail('Facebook Pixel: No valid ecommerce/eventModel object found.');
	}

	if (data.stripPrefix && gtmEvent.indexOf('_') > -1) {
		gtmEvent = gtmEvent.split('_').slice(1).join('_');
	}
	...
	// Start with default GA4 mapping
	var mappedEvent = ga4ToFbEventMap[gtmEvent] || 'PageView';

	// If remapping is enabled, override with custom table map
	if (data.enableEventRemap && data.eventNameMap && data.eventNameMap.length) {
		var eventRemap = makeTableMap(data.eventNameMap, 'siteEventName', 'ga4EventName');
		if (eventRemap[gtmEvent]) {
			mappedEvent = eventRemap[gtmEvent];
		}
	}

	eventName = mappedEvent;

	const contentIdKey = data.eecCustomContentIdMap ? data.eecCustomContentId : 'item_id';
	const currencyKey = data.eecCustomCurrencyMap ? data.eecCustomCurrency : 'currency';

	if (format == 'ga4' || format == 'eventModel') {
		if (!ecommerce.items || getType(ecommerce.items) !== 'array') {
			return fail('Facebook Pixel: Missing or invalid "items" array.');
		}

		let value = makeNumber(ecommerce.value || 0);
		if (!value || value === 0) {
			value = ecommerce.items.reduce(function(acc, item) {
				return acc + (makeNumber(item.price || 0) * makeNumber(item.quantity || 1));
			}, 0);
		}
		...
		eecObjectProps = {
			...
			contents: ecommerce.items.map(function(item) {
				return {
					id: item[contentIdKey] || item.id || item.sku || 'unknown',
					quantity: makeNumber(item.quantity || 1)
				};
			}),
			value: math.round(value * 100) / 100,
			currency: ecommerce[currencyKey] || 'USD'
		};

		if (eventName === 'InitiateCheckout' || eventName === 'Purchase') {
			eecObjectProps.num_items = ecommerce.items.reduce(function(acc, item) {
				return acc + makeNumber(item.quantity || 1);
			}, 0);
		}
	}
	...
	if (format == 'ua') {
		...
		if (!action || !ecommerce[action] || !ecommerce[action].products || getType(ecommerce[action].products) !== 'array') {
			return fail('Facebook Pixel: Invalid ecommerce UA-style structure.');
		}

		let value = 0;
		if (action === 'purchase' && ecommerce[action].actionField && ecommerce[action].actionField.revenue) {
			value = makeNumber(ecommerce[action].actionField.revenue);
		} else {
			value = ecommerce[action].products.reduce(function(acc, cur) {
				return acc + (makeNumber(cur.price || 0) * makeNumber(cur.quantity || 1));
			}, 0);
		}

		eecObjectProps = {
			content_type: 'product',
			contents: ecommerce[action].products.map(function(prod) {
				return {
					id: prod[contentIdKey] || prod.id || prod.sku || 'unknown',
					quantity: makeNumber(prod.quantity || 1)
				};
			}),
			value: math.round(value * 100) / 100,
			currency: ecommerce[currencyKey] || ecommerce.currencyCode || 'USD'
		};

		if (eventName === 'InitiateCheckout' || eventName === 'Purchase') {
			eecObjectProps.num_items = ecommerce[action].products.reduce(function(acc, cur) {
				return acc + makeNumber(cur.quantity || 1);
			}, 0);
		}
	}

	log('📦 Final EEC Object:', eecObjectProps);
}
...
const command = standardEventNames.indexOf(eventName) > -1 ? 'trackSingle' : 'trackSingleCustom';
const consent = data.consent === false ? 'revoke' : 'grant';

// Init fbq
const getFbq = function() {
	var fbq = copyFromWindow('fbq');
	if (fbq) return fbq;
	setInWindow('fbq', function() {
		var callMethod = copyFromWindow('fbq.callMethod.apply');
		if (callMethod) callInWindow('fbq.callMethod.apply', null, arguments);
...
pixelIds.split(',').forEach(function(pixelId) {
	if (initIds.indexOf(pixelId) === -1) {
		if (data.disableAutoConfig) fbq('set', 'autoConfig', false, pixelId);
		if (data.disablePushState) setInWindow('fbq.disablePushState', true);
	...
	//log('🚀 fbq Call:', command, pixelId, eventName, finalObjectProps);
